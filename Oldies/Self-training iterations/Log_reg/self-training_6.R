...
train |>
  count(label) # misinfo = 23006, non misinfo = 252772

################################################################################
275778/(table(train$label)[1] * 2) # 5.99361     
275778/(table(train$label)[2] * 2) # 0.5455074     

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 5.99361, 0.5455074),
...
cm_lr$byClass["F1"] # 0.995383         
cm_lr$byClass["Precision"] # 0.9991418       
cm_lr$byClass["Recall"] # 0.9916525      

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

# saveRDS(lr_final_fit, "~/INORK_ST/log_reg_round_6.RDS")
################################################################################
