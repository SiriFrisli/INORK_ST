...
################################################################################
covid <- readRDS("E:/Data/Training samples/misinformation_class_11.RDS")
...
################################################################################
376050/(table(train$label)[1] * 2) # 5.349979           
376050/(table(train$label)[2] * 2) # 0.5515466          
...
cm_lr$byClass["F1"] # 0.9937854         
cm_lr$byClass["Precision"] # 0.9993304         
cm_lr$byClass["Recall"] # 0.9883015    

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

# saveRDS(lr_final_fit, "~/INORK_ST/log_reg_round_12.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 44097, nonmisinfo = 419099
...
covid_predicted |>
  count(label) # misinfo = 44632, nonmisinfo = 427120

saveRDS(covid_predicted, "E:/Data/Training samples/misinformation_class_12.RDS")
