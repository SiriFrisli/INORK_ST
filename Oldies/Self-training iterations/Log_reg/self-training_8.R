...
################################################################################
covid <- readRDS("E:/Data/Training samples/misinformation_class_7.RDS")
...
train |>
  count(label) # misinfo = 30384, non misinfo = 320143

################################################################################
350527/(table(train$label)[1] * 2) # 5.768283       
350527/(table(train$label)[2] * 2) # 0.5474538       

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 5.768283, 0.5474538),
...
cm_lr$byClass["F1"] # 0.9942658     
cm_lr$byClass["Precision"] # 0.9994819     
cm_lr$byClass["Recall"] # 0.989104  

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

# saveRDS(lr_final_fit, "~/INORK_ST/log_reg_round_8.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 40202, nonmisinfo = 406299
...
covid_predicted |>
  count(label) # misinfo = 40697, nonmisinfo = 413242

saveRDS(covid_predicted, "E:/Data/Training samples/misinformation_class_8.RDS")
