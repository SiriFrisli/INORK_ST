...
################################################################################
covid <- readRDS("E:/Data/Training samples/misinformation_class_9.RDS")
...
train |>
  count(label) # misinfo = 33862, non misinfo = 336904

################################################################################
370766/(table(train$label)[1] * 2) # 5.474662         
370766/(table(train$label)[2] * 2) # 0.5502547         

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 5.474662, 0.5502547),
...
cm_lr$byClass["F1"] # 0.9937188       
cm_lr$byClass["Precision"] # 0.9998819       
cm_lr$byClass["Recall"] # 0.9876313    

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

# saveRDS(lr_final_fit, "~/INORK_ST/log_reg_round_10.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 42940, nonmisinfo = 416055
...
covid_predicted |>
  count(label) # misinfo = 43452, nonmisinfo = 423746

saveRDS(covid_predicted, "E:/Data/Training samples/misinformation_class_10.RDS")
