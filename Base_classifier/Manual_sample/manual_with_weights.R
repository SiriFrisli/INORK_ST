...
841/(table(train$label)[1] * 2) # 6.469231 
841/(table(train$label)[2] * 2) # 0.5418814 
...
svm_tune <- tune(svm, 
                 ...
                 ranges = list(gamma=c(0, 0.001, 0.01, 0.1, 1), 
                               cost = c(0.1, 1, 5, 10)),
                 class.weights = c("misinfo" = 6.469231, "non.misinfo" = 0.5418814),
                 probability = TRUE,
...
# Creating the weights and recipe (same will be used for GBDT)
set.seed(8008)
...
cm_bt$byClass["F1"] # 0.06451613    
cm_bt$byClass["Precision"] # 0.125 
cm_bt$byClass["Recall"] # 0.04347826    
