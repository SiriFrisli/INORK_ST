...
train |>
  count(label) # misinfo = 5615, non misinfo = 62438

################################################################################
68053/(table(train$label)[1] * 2) # 6.059929  
68053/(table(train$label)[2] * 2) # 0.5449646  

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 6.059929, 0.5449646),
...
cm_lr$byClass["F1"] # 0.9871035      
cm_lr$byClass["Precision"] # 0.9971831    
cm_lr$byClass["Recall"] # 0.9772257   

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

# saveRDS(lr_final_fit, "~/INORK_ST/log_reg_round_3.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 14328, nonmisinfo = 138839
