...
################################################################################
covid <- readRDS("E:/Data/Training samples/misinformation_class_10.RDS")
...
train |>
  count(label) # misinfo = 34581, non misinfo = 339177

################################################################################
373758/(table(train$label)[1] * 2) # 5.404095          
373758/(table(train$label)[2] * 2) # 0.5509778          

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 5.404095, 0.5509778),
...
cm_lr$byClass["F1"] # 0.9944472        
cm_lr$byClass["Precision"] # 0.9993236        
cm_lr$byClass["Recall"] # 0.9896182    

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

# saveRDS(lr_final_fit, "~/INORK_ST/log_reg_round_11.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 43601, nonmisinfo = 418185
...
covid_predicted |>
  count(label) # misinfo = 44106, nonmisinfo = 425957

saveRDS(covid_predicted, "E:/Data/Training samples/misinformation_class_11.RDS")
