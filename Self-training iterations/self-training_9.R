...
################################################################################
covid <- readRDS("E:/Data/Training samples/misinformation_class_8.RDS")
...
train |>
  count(label) # misinfo = 32458, non misinfo = 330693

################################################################################
363151/(table(train$label)[1] * 2) # 5.594168        
363151/(table(train$label)[2] * 2) # 0.5490757        

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 5.594168, 0.5490757),
...
cm_lr$byClass["F1"] # 0.9932424      
cm_lr$byClass["Precision"] # 0.999029      
cm_lr$byClass["Recall"] # 0.9875225   

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

# saveRDS(lr_final_fit, "~/INORK_ST/log_reg_round_9.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 41785, nonmisinfo = 414024
...
covid_predicted |>
  count(label) # misinfo = 42327, nonmisinfo = 421131

saveRDS(covid_predicted, "E:/Data/Training samples/misinformation_class_9.RDS")
