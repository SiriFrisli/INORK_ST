...
train |>
  count(label) # misinfo = 17565, non misinfo = 176215

################################################################################
193780/(table(train$label)[1] * 2) # 5.516083    
193780/(table(train$label)[2] * 2) # 0.5498397    

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 5.516083, 0.5498397),
...
cm_lr$byClass["F1"] # 0.9932809        
cm_lr$byClass["Precision"] # 0.9986261      
cm_lr$byClass["Recall"] # 0.9879928     

lr_preds |>
  conf_mat(truth = label, estimate = .pred_class) |> 
  autoplot(type = "heatmap") 

# saveRDS(lr_final_fit, "~/INORK_ST/log_reg_round_5.RDS")
################################################################################
...
lr_preds_all_filtered_label |>
  count(label) # misinfo = 28649, nonmisinfo = 312310
...
covid_predicted |>
  count(label) # misinfo = 28832, nonmisinfo = 315891
